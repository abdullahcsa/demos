...
public class TimeConverterServiceTest {
    ...
    // Test special cases (multiples of 5)
    @Test
    @DisplayName("Should use SpecialTimeConverter for 2:05")
    public void testSpecialConverterForFivePastTwo() {
        Time time = new Time(2, 5);
        assertEquals("five past two", service.convert(time));
    }

    @Test
    @DisplayName("Should use SpecialTimeConverter for 6:25")
    public void testSpecialConverterForTwentyFivePastSix() {
        Time time = new Time(6, 25);
        assertEquals("twenty five past six", service.convert(time));
    }

    @Test
    @DisplayName("Should use SpecialTimeConverter for 8:40")
    public void testSpecialConverterForTwentyToNine() {
        Time time = new Time(8, 40);
        assertEquals("twenty to nine", service.convert(time));
    }

    @Test
    @DisplayName("Should use SpecialTimeConverter for 11:55")
    public void testSpecialConverterForFiveToTwelve() {
